  ...
  return (
    <article className="publication-scroll h-full overflow-y-auto rounded-xl border border-[#e7e1d9] bg-white p-5 shadow-[0_3px_15px_rgba(23,34,55,0.06)]">
      <div className="flex flex-col gap-4 border-b border-[#ece8e1] pb-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-x-5 gap-y-2 text-base text-slate-500 sm:text-2xl">
        ...
        <p className="text-sm text-slate-400">{formatDate(vacante.fechaPublicacion)}</p>
      </div>
      ...
      <div className="mt-5 px-1 text-slate-600">
        <h2 className="text-2xl font-semibold text-slate-900 sm:text-3xl">Descripcion completa del empleo</h2>
        <p className="mt-2 whitespace-pre-line text-base leading-relaxed sm:text-xl">{vacante.descripcion}</p>

        {requirements.length > 0 ? (
          <>
            <h3 className="mt-4 text-xl font-semibold text-slate-800 sm:text-2xl">Requisitos / responsabilidades</h3>
            <ul className="mt-2 list-disc space-y-1 pl-6 text-base leading-relaxed sm:text-lg">
